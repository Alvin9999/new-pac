...
无需域名。有域名也行。按照提示操作安装，全程回车即可。搭建好后默认有6个节点，3个vless节点，1个vmess节点，1个trojan节点，1个ss节点，第3第4第5个节点自动配置了cloudflare的cdn。鼠标往上翻，根据客户端来复制对应的一键导入链接。支持V2rayN、NekoBox、Shadowrocket、Clash Meta、Sing-box客户端。
...
VPS IP、Argo域名、Reality的端口号、优选域名都可以回车。等待2～5分钟。
...
第3第4第5个节点自动配置了cloudflare的cdn。脚本内置了5个cloudflare的优选域名，如果域名被墙了，可以更换其它优选域名，更换优选域名可以不用重新搭建，直接在客户端里面将域名更改即可。除了填写优选域名以外，还可以用优选的cloudflare的ip，比如这里的[CloudFlare优质IP（不定期更新）](https://github.com/Alvin9999/new-pac/blob/master/CloudFlare%E4%BC%98%E8%B4%A8IP)
