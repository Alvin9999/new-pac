...
无需域名。搭建好后有9个节点，包括vmess、vless、ss、hysteria2节点。

其中有3个节点分别是vmess-ws(tls)+Argo节点、vless-ws(tls)+Argo、trojan-tls+argo节点自动配置了cloudflare的cdn。搭建的过程中可以选择cloudflare的优选域名，如果域名被墙了或者速度不理想，直接在客户端里面将域名更改即可。除了填写优选域名以外，还可以用优选的cloudflare的ip，比如这里的[CloudFlare优质IP（不定期更新）](https://github.com/Alvin9999/new-pac/blob/master/CloudFlare%E4%BC%98%E8%B4%A8IP) ，操作方法：比如在v2rayN客户端里面将这个节点的地址（address）那一栏改成优选的cloudflare的ip。
