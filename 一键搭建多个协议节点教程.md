...
无需域名。有域名也行。按照提示操作安装，全程回车即可。搭建好后默认有6个节点，3个vless节点，1个vmess节点，1个trojan节点，1个ss节点，带域名的节点自动配置了cloudflare的cdn。鼠标往上翻，根据客户端来复制对应的一键导入链接。支持V2rayN、NekoBox、Shadowrocket、Clash Meta、Sing-box客户端。
